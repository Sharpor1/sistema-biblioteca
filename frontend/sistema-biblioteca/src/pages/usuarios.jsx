...
export default function Usuarios() {
  ...
  function handleChange(e) {
    ...
    setForm((s) => ({
      ...s,
      [name]: name === 'rol' ? Number(value) : (name === 'rut' ? formatRut(value) : value)
    }));
                  ...
                  <div className="space-y-3">
                    {currentUsers.length === 0 ? (
                      ...
                      currentUsers.map((u) => (
                        <div
                          key={u.id}
                          className={`border rounded p-3 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-colors ${u.estado === 'bloqueado' ? 'border-rose-300 bg-rose-50' : ''
                            }`}
                          onClick={async () => {
                            setSelectedUser(u);
            ...
            <div className="bg-white rounded-xl shadow-lg max-w-3xl w-full mx-4 my-auto">
              ...
              <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div className="bg-slate-50 p-4 rounded-lg">
                  ...
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${selectedUser.estado === 'activo' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                    }`}>
                        ...
                        return (
                          <div key={prestamo.idPrestamo} className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                            <div className="flex justify-between items-start">
                              ...
                              <span className={`px-2 py-1 text-xs font-semibold rounded-full ${estaRetrasado ? 'bg-rose-100 text-rose-800' : 'bg-emerald-100 text-emerald-800'
                                }`}>
