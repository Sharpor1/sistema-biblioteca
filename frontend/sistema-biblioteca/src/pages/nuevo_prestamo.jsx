...
export default function NuevoPrestamo() {
  ...
  async function registrarPrestamo(e) {
    ...
    if (dias <= 0) {
      showNotification('Los días de préstamo deben ser mayor a 0', 'warning');
      return;
    }
    if (usuarioValid.diasMax && dias > usuarioValid.diasMax) {
      showNotification(`Los días no pueden exceder el máximo permitido (${usuarioValid.diasMax} días)`, 'warning');
      return;
    }
    if (usuarioValid.diasMin && dias < usuarioValid.diasMin) {
      showNotification(`Los días no pueden ser menores al mínimo permitido (${usuarioValid.diasMin} días)`, 'warning');
      return;
    }
    ...
    try {
      ...
      showNotification('No se pudo registrar el préstamo: ' + errorMsg, 'error');
    }
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
              ...
              <button
                ...
                className={`w-full px-6 py-3 rounded-lg font-semibold text-base transition-all ${canRegister
                  ...
                  usuarios
                    ...
                    .map((u) => {
                      ...
                      return (
                        <div
                          key={u.rut}
                          className={`border rounded p-3 cursor-pointer transition-colors ${tieneProblemas
                      ...
                      return (
                        <div key={libro.idLibro} className={`border rounded-lg overflow-hidden ${sinStock ? 'border-slate-300 bg-slate-50' : 'border-slate-200'}`}>
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <span className={`px-2 py-1 rounded-full text-xs font-semibold ${sinStock ? 'bg-slate-200 text-slate-600' : 'bg-emerald-100 text-emerald-800'
                                }`}>
